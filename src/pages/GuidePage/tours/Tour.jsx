...
import { createTourApi, getTourGuide, uploadFile } from "../../../api/tourAPI";
...
export default function ManageTourGuide() {
  ...
  const formatTours = useCallback((tourData) => {
    return tourData.map((tour) => ({
      ...
      file: tour.file || "", // Fixed: removed reference to undefined imageFile
      image:
        tour.images?.[0] ||
  ...
  const validateForm = () => {
    ...
    newTour.locations.forEach((loc, index) => {
      if (!loc.name)
        errors[`locationName${index}`] = `Tên địa điểm ${
          index + 1
        } là bắt buộc`;
      if (loc.images.length === 0)
        errors[`locationImages${index}`] = `Hình ảnh địa điểm ${
          index + 1
        } là bắt buộc`;
      if (!loc.latitude || !loc.longitude)
        errors[`locationCoords${index}`] = `Tọa độ địa điểm ${
          index + 1
        } là bắt buộc`;
    });
